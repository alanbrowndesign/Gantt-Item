...
function Widget() {
  const [itemWidth, setItemwidth] = useSyncedState("itemcount", itemBlock);
  const [count, setCount] = useSyncedState("count", 79);
  const [text, setText] = useSyncedState("text", "Task name");
  ...
  return (
    ...
    >
      ...
      <Frame
        ...
        cornerRadius={99}
        onClick={() => {
          setItemwidth(itemWidth + itemBlock + 4);
        }}
      />
    </AutoLayout>
  );
